...
function ChatPage() {
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: 'bot',
      content: 'Hello! I can help you analyze your documents. Please upload a document to get started.',
  ...
  const [documentMetadata, setDocumentMetadata] = useState(null); // Store file URI and other metadata
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorResponse = {
        id: Date.now() + 1,
        type: 'bot',
        content: 'I apologize, but I encountered an error. Please make sure you have uploaded a document and try again.',
        timestamp: new Date().toLocaleTimeString()
      };
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      setDocumentMetadata({
        fileUri: data.analysis.fileUri,
        fileName: data.analysis.fileName,
        mimeType: data.analysis.mimeType
      });
      setDocumentInfo(data.analysis);

      // Add a message about successful document analysis
      const analysisMessage = {
        id: Date.now() + 2,
        type: 'bot',
        content: `I've successfully analyzed "${file.name}". The document covers topics like: ${data.analysis.keyTopics.slice(0, 3).join(', ')}. You can now ask me questions about this document!`,
        timestamp: new Date().toLocaleTimeString()
      };
      ...
      const errorMessage = {
        id: Date.now() + 2,
        type: 'bot',
        content: 'I encountered an error while analyzing the document. Please try uploading the file again or use a different format.',
        timestamp: new Date().toLocaleTimeString()
      };
  ...
  const removeFile = (fileId) => {
    ...
    if (uploadedFiles.length === 1) {
      ...
      const removalMessage = {
        id: Date.now(),
        type: 'bot',
        content: 'Document removed. Please upload a new document to continue our conversation.',
        timestamp: new Date().toLocaleTimeString()
      };
